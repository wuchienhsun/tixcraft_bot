        ...
        with open(config_filepath) as json_data:
            ...
            if args.kktix_password:
                config_dict["advanced"][
                    "kktix_password_plaintext"] = args.kktix_password

            if args.proxy_server:
                config_dict["advanced"][
                    "proxy_server_port"] = args.proxy_server
...
def get_favoriate_extension_path(webdriver_path, config_dict):
    ...
    extension_list.append(
        os.path.join(webdriver_path, CONST_MAXBOT_EXTENSION_NAME + ".crx"))
    extension_list.append(
        os.path.join(webdriver_path, CONST_MAXBLOCK_EXTENSION_NAME + ".crx"))
...
def load_chromdriver_normal(config_dict, driver_type):
    ...
    if not os.path.exists(chromedriver_path):
        print(
            "Please download chromedriver and extract zip to webdriver folder from this url:"
        )
        ...
        try:
            ...
            if show_debug_message:
                print(exc)
            left_part = error_message.split(
                "Stacktrace:")[0] if "Stacktrace:" in error_message else None
...
def load_chromdriver_uc(config_dict):
    ...
    if driver is None:
        ...
        if os.path.exists(chromedriver_path):
            ...
            print(
                "you need manually download ChromeDriver to webdriver folder.")
...
def get_driver_by_config(config_dict):
    ...
    if config_dict["browser"] in ["chrome", "brave"]:
        ...
        if config_dict["webdriver_type"] == CONST_WEBDRIVER_TYPE_SELENIUM:
            driver = load_chromdriver_normal(config_dict,
                                             config_dict["webdriver_type"])
    ...
    if config_dict["browser"] == "firefox":
        ...
        if "macos" in platform.platform().lower():
            if "arm64" in platform.platform().lower():
                chromedriver_path = os.path.join(webdriver_path,
                                                 "geckodriver_arm")
...
def force_press_button_iframe(driver,
                              f,
                              select_by,
                              select_query,
                              force_submit=True):
...
def remove_attribute_tag_by_selector(driver,
                                     select_query,
                                     class_name,
                                     more_script=""):
    element_script = "eachItem.removeAttribute('" + class_name + "');"
    javascript_tag_by_selector(driver,
                               select_query,
                               element_script,
                               more_script=more_script)


def remove_class_tag_by_selector(driver,
                                 select_query,
                                 class_name,
                                 more_script=""):
...
def javascript_tag_by_selector(driver,
                               select_query,
                               element_script,
                               more_script=""):
    try:
...
def tixcraft_date_auto_select(driver, url, config_dict, domain_name):
    ...
    pass_date_is_sold_out_enable = config_dict["tixcraft"][
        "pass_date_is_sold_out"]
    auto_reload_coming_soon_page_enable = config_dict["tixcraft"][
        "auto_reload_coming_soon_page"]

    # PS: for big events, check sold out text maybe not helpful, due to database is too busy.
    sold_out_text_list = [
        "選購一空", "已售完", "No tickets available", "Sold out", "空席なし", "完売した"
    ]
    ...
    if not area_list is None:
        ...
        if area_list_count > 0:
            ...
            for row in area_list:
                ...
                if len(row_text) > 0:
                    if util.reset_row_text_if_match_keyword_exclude(
                            config_dict, row_text):
                        row_text = ""
                ...
                if len(row_text) > 0:
                    ...
                    if is_match_all_coming_soon_condiction:
                        if show_debug_message:
                            print("match coming soon condiction at row:",
                                  row_text)
                        is_coming_soon = True
                        ...
                        if pass_date_is_sold_out_enable:
                            for sold_out_item in sold_out_text_list:
                                ...
                                if sold_out_item in row_text_right_part:
                                    ...
                                    if show_debug_message:
                                        print(
                                            "match sold out text: %s, skip this row."
                                            % (sold_out_item))
                                    break
            ...
            if len(date_keyword) == 0:
                ...
                matched_blocks = util.get_matched_blocks_by_keyword(
                    config_dict, auto_select_mode, date_keyword,
                    formated_area_list)
                            ...
                            if not area_item_font_el is None:
                                ...
                                for check_item in CONT_STRING_1_SEATS_REMAINING:
                                    if check_item in font_el_text:
                                        if show_debug_message:
                                            print(
                                                "match pass 1 seats remaining 1 full text:",
                                                row_text)
                                            print(
                                                "match pass 1 seats remaining 2 font text:",
                                                font_el_text)
                                        is_append_this_row = False
                            else:
            ...
            if row_is_enabled:
                try:
                    ...
                    if "price" in zone_info[row]:
                        row_text += " " + zone_info[row]["price"][0][
                            "ticketPrice"]
                except Exception as exc:
                    if show_debug_message:
    ...
    if False:
        if not select is None:
            try:
                ...
                driver.execute_script("$(\"input[type='select']\").val(\"" +
                                      ticket_number + "\");")
                is_ticket_number_assigned = True
            except Exception as exc:
                print(
                    "jQuery select_by_visible_text ticket_number fail (after click.)"
                )
                print(exc)
...
def fill_common_verify_form(driver, config_dict, inferred_answer_string,
                            fail_list, input_text_css, next_step_button_css,
                            submit_by_enter, check_input_interval):
    show_debug_message = True  # debug.
    show_debug_message = False  # online
    ...
    if not is_multi_question_mode:
                ...
                if len(inputed_value_1) == 0:
                    try:
                        ...
                        driver.execute_script(
                            "if(!(document.activeElement === arguments[0])){arguments[0].focus();}",
                            form_input_1)
                        # solution 2: selenium.
                        #form_input_1.click()
        ...
        try:
            ...
            if len(next_step_button_css) > 0:
                is_button_clicked = press_button(driver, By.CSS_SELECTOR,
                                                 next_step_button_css)
...
def ticketmaster_promo(driver, config_dict, fail_list):
    ...
    return tixcraft_input_check_code(driver, config_dict, fail_list,
                                     question_selector)
...
def tixcraft_input_check_code(driver, config_dict, fail_list,
                              question_selector):
    show_debug_message = True  # debug.
    show_debug_message = False  # online
    ...
    question_text = get_text_by_selector(driver, question_selector,
                                         'innerText')
    if len(question_text) > 0:
        ...
        is_answer_sent, fail_list = fill_common_verify_form(
            driver, config_dict, inferred_answer_string, fail_list,
...
def tixcraft_toast(driver, message):
    ...
    try:
        ...
        if not toast_element is None:
            driver.execute_script("arguments[0].innerHTML='%s';" % message,
                                  toast_element)
    except Exception as exc:
...
def tixcraft_keyin_captcha_code(driver, answer="", auto_submit=False):
    ...
    try:
        form_verifyCode = driver.find_element(By.CSS_SELECTOR,
        ...
        if is_visible:
            ...
            if inputed_value == "":
                try:
                    ...
                    print(
                        "click form_verifyCode fail, trying to use javascript."
                    )
                    # plan B
                    try:
                        driver.execute_script(
...
def tixcraft_get_ocr_answer(driver, ocr, ocr_captcha_image_source,
    ...
    if not ocr is None:
        ...
        if ocr_captcha_image_source == CONST_OCR_CAPTCH_IMAGE_SOURCE_CANVAS:
            ...
            try:
                ...
                image_element = driver.find_elements(By.CSS_SELECTOR,
                                                     my_css_selector)
            except Exception as exc:
                pass
            ...
            if not image_element is None:
                ...
                try:
                    ...
                    if img_base64 is None:
                        if not Captcha_Browser is None:
                            print(
                                "canvas get image fail, use plan_b: NonBrowser"
                            )
                            img_base64 = base64.b64decode(
                                Captcha_Browser.request_captcha())
                except Exception as exc:
...
def tixcraft_auto_ocr(driver, ocr, away_from_keyboard_enable, previous_answer,
    ...
    if is_input_box_exist:
        ...
        ocr_answer = tixcraft_get_ocr_answer(driver, ocr,
...
def tixcraft_ticket_main_agree(driver, config_dict):
    print("tixcraft_ticket_main_agree")
    is_finish_checkbox_click = False
    for i in range(3):
        is_finish_checkbox_click = check_checkbox(driver, By.CSS_SELECTOR,
                                                  '#TicketForm_agree')
...
def get_tixcraft_ticket_select_by_keyword(driver, config_dict,
                                          area_keyword_item):
    show_debug_message = True  # debug.
    show_debug_message = False  # online
...
def tixcraft_assign_ticket_number(driver, config_dict):
    ...
    try:
        form_select_list = driver.find_elements(By.CSS_SELECTOR,
                                                '.mobile-select')
    except Exception as exc:
    ...
    if form_select is None:
        try:
            form_select = driver.find_element(By.CSS_SELECTOR,
...
def tixcraft_ticket_main(driver, config_dict, ocr, Captcha_Browser,
                         domain_name):
    ...
    if is_ticket_number_assigned:
        tixcraft_ticket_main_ocr(driver, config_dict, ocr, Captcha_Browser,
                                 domain_name)


def tixcraft_ticket_main_ocr(driver, config_dict, ocr, Captcha_Browser,
                             domain_name):
    ...
    if not config_dict["ocr_captcha"]["enable"]:
        ...
        for redo_ocr in range(19):
            is_need_redo_ocr, previous_answer, is_form_sumbited = tixcraft_auto_ocr(
                driver, ocr, away_from_keyboard_enable, previous_answer,
                Captcha_Browser, ocr_captcha_image_source, domain_name)
            if is_form_sumbited:
                # start next loop.
                break
...
def kktix_travel_price_list(driver, config_dict, kktix_area_auto_select_mode,
                            kktix_area_keyword):
    show_debug_message = config_dict["advanced"]["verbose"]
    ticket_number = config_dict["ticket_number"]
    ...
    try:
        ticket_price_list = driver.find_elements(By.CSS_SELECTOR,
            ...
            if len(row_text) > 0:
                if ticket_number > 1:
                    ...
                    if ' danger' in row_html and ' Left ' in row_html:
                        ...
                        if len(tmp_array) > 0:
                            tmp_ticket_count = tmp_array[len(tmp_array) -
                                                         1].strip()
                            if tmp_ticket_count.isdigit():
                                if show_debug_message:
            ...
            if current_ticket_number == "0":
                try:
                    ...
                    try:
                        ...
                        print(
                            "asssign ticket number to ticket-price still failed."
                        )
                        pass
            else:
                if show_debug_message:
...
def kktix_double_check_all_text_value(driver, ticket_number):
    ...
    try:
        ...
        ticket_price_input_list = driver.find_elements(By.CSS_SELECTOR,
                                                       my_css_selector)
    except Exception as exc:
        pass
...
def set_kktix_control_label_text(driver, config_dict):
    ...
    check_input_interval = 0.2
    is_answer_sent, fail_list = fill_common_verify_form(
        driver, config_dict, inferred_answer_string, fail_list, input_text_css,
...
def kktix_reg_captcha(driver, config_dict, fail_list, registrationsNewApp_div):
    ...
    question_text = get_text_by_selector(driver, 'div.custom-captcha-inner p',
                                         'innerText')
    if len(question_text) > 0:
...
def kktix_reg_new_main(driver, config_dict, fail_list, played_sound_ticket):
    ...
    try:
        registrationsNewApp_div = driver.find_element(By.CSS_SELECTOR,
                                                      '#registrationsNewApp')
    except Exception as exc:
        pass
        ...
        if len(area_keyword) > 0:
            ...
            for area_keyword_item in area_keyword_array:
                ...
                if is_ticket_number_assigned:
                    ...
                    if show_debug_message:
                        print("is_need_refresh for keyword:",
                              area_keyword_item)
            ...
            if len(date_keyword) == 0 and len(area_keyword_item) == 0:
                ...
                for row in formated_area_list:
                    ...
                    try:
                        ...
                        td_date = row.find_element(By.CSS_SELECTOR,
                                                   my_css_selector)
                        ...
                        td_area = row.find_element(By.CSS_SELECTOR,
                                                   my_css_selector)
...
def fami_activity(driver):
    ...
    try:
        fami_start_to_buy_button = driver.find_element(By.CSS_SELECTOR,
                                                       '#buyWaiting')
    except Exception as exc:
        pass
...
def fami_date_auto_select(driver, config_dict, last_activity_url):
    ...
    if not target_area is None:
        ...
        for i in range(3):
            ...
            try:
                my_css_selector = "button"
                el_btn = target_area.find_element(By.CSS_SELECTOR,
                                                  my_css_selector)
            except Exception as exc:
                pass
...
def fami_home_auto_select(driver, config_dict, last_activity_url):
    ...
    if ticket_el is None:
        ...
        is_date_assign_by_bot = fami_date_auto_select(driver, config_dict,
                                                      last_activity_url)
        is_price_assign_by_bot = fami_date_to_area(driver, config_dict,
                                                   last_activity_url)
...
def urbtix_purchase_ticket(driver, config_dict):
    ...
    is_date_assign_by_bot = urbtix_date_auto_select(
        driver, date_auto_select_mode, date_keyword,
        auto_reload_coming_soon_page_enable)
...
def urbtix_ticket_number_auto_select(driver, config_dict):
    ...
    ticket_price_input = None
    try:
        ticket_price_input = driver.find_element(By.CSS_SELECTOR,
                                                 "input.ticket-count")
    except Exception as exc:
        pass
        ...
        if len(current_ticket_number) > 0:
            if current_ticket_number == "0":
                try:
                    ...
                    print(
                        "asssign ticket number to ticket-count field Exception:"
                    )
                    print(exc)
                    try:
                        ticket_price_input.clear()
                        ...
                        print(
                            "asssign ticket number to ticket-count still failed."
                        )
                        pass
            else:
                # already assigned.
    ...
    if is_ticket_number_assigned:
        ...
        try:
            ...
            el_ticket_count = driver.find_element(By.CSS_SELECTOR,
                                                  my_css_selector)
...
def urbtix_uncheck_adjacent_seat(driver, config_dict):
    ...
    try:
        ...
        el_checkbox_icon = driver.find_element(By.CSS_SELECTOR,
                                               my_css_selector)
...
def cityline_ticket_number_auto_select(driver, config_dict):
    ...
    return assign_ticket_number_by_select(driver, config_dict, by_method,
                                          selector_string)
...
def assign_ticket_number_by_select(driver, config_dict, by_method,
                                   selector_string):
    show_debug_message = True  # debug.
    show_debug_message = False  # online
                            ...
                            if not area_seat_el is None:
                                ...
                                if seat_text.isdigit():
                                    ...
                                    if seat_int < config_dict["ticket_number"]:
                                        ...
                                        if show_debug_message:
                                            print(
                                                "skip not enought ticket number area at row_text:",
                                                row_text)
                                        row_text = ""
                        except Exception as exc:
                            if show_debug_message:
...
def facebook_login(driver, account, password):
    ...
    if is_email_sent:
        is_password_sent = assign_text(driver,
                                       By.CSS_SELECTOR,
                                       '#pass',
                                       password,
                                       submit=True)
...
def cityline_login(driver, account, password):
    is_email_sent = assign_text(driver,
                                By.CSS_SELECTOR,
                                'input[type="text"]',
                                account,
                                submit=True)
...
def urbtix_login(driver, account, password):
    ...
    el_email = None
    try:
        el_email = driver.find_element(By.CSS_SELECTOR,
                                       'input[name="loginId"]')
    except Exception as exc:
    ...
    if is_email_sent:
        try:
            el_pass = driver.find_element(By.CSS_SELECTOR,
                                          'input[name="password"]')
        except Exception as exc:
            pass
        ...
        if len(zone_string) > 0:
            ...
            try:
                ...
                if not zone_info is None:
                    ticketmaster_area_auto_select(driver, config_dict,
                                                  zone_info)
            except Exception as exc:
                if show_debug_message:
                    print(exc)
...
def ticketmaster_get_ticketPriceList(driver, config_dict):
    ...
    try:
        ...
        div_mapContainer = driver.find_element(By.CSS_SELECTOR,
                                               my_css_selector)
    except Exception as exc:
        if show_debug_message:
            print('fail to find my_css_selector:', my_css_selector)
    ...
    if not div_mapContainer is None:
        ...
        try:
            ...
            div_loadingmap = driver.find_element(By.CSS_SELECTOR,
                                                 my_css_selector)
        ...
        if not is_loading:
            try:
                ...
                table_select = driver.find_element(By.CSS_SELECTOR,
                                                   my_css_selector)
            except Exception as exc:
                if show_debug_message:
                    print('fail to find my_css_selector:', my_css_selector)
...
def ticketmaster_assign_ticket_number(driver, config_dict):
    ...
    if not table_select is None:
        ...
        try:
            ...
            form_select = table_select.find_element(By.CSS_SELECTOR,
                                                    my_css_selector)
        except Exception as exc:
            if show_debug_message:
                print('my_css_selector:', my_css_selector)
...
def ticketmaster_captcha(driver, config_dict, ocr, Captcha_Browser,
                         domain_name):
    print("ticketmaster_captcha")
    show_debug_message = True  # debug.
    # show_debug_message = False      # online
...
def tixcraft_main(driver, url, config_dict, ocr, Captcha_Browser):
    ...
    home_url_list = [
        ...
        'https://www.indievox.com/', 'https://teamear.tixcraft.com/activity',
        'https://ticketmaster.sg/', 'https://ticketmaster.com/'
    ]
    ...
    if '/ticket/check-captcha/' in url:
        domain_name = url.split('/')[2]
        ticketmaster_captcha(driver, config_dict, ocr, Captcha_Browser,
                             domain_name)
    ...
    if '/ticket/ticket/' in url:
        domain_name = url.split('/')[2]
        tixcraft_ticket_main(driver, config_dict, ocr, Captcha_Browser,
                             domain_name)
    ...
    if '/ticket/checkout' in url:
        if not tixcraft_dict["start_time"] is None:
            if not tixcraft_dict["done_time"] is None:
                bot_elapsed_time = tixcraft_dict["done_time"] - tixcraft_dict[
                    "start_time"]
...
def kktix_paused_main(driver, url, config_dict):
    ...
    if '/users/sign_in?' in url:
        ...
        kktix_password = config_dict["advanced"][
    ...
    if not is_url_contain_sign_in:
        if '/registrations/new' in url:
            ...
            recaptcha_div = None
            try:
                recaptcha_div = driver.find_element(By.CSS_SELECTOR,
                                                    '.event-captcha-info')
            except Exception as exc:
                pass
            ...
            if not recaptcha_div is None:
                ...
                class_name = 'disabled'
                remove_attribute_tag_by_selector(driver, select_query,
                                                 class_name)
...
def kktix_main(driver, url, config_dict):
        ...
        if '/registrations/new' in url:
            ...
            if not is_dom_ready:
                ...
                if config_dict["kktix"]["auto_fill_ticket_number"]:
                    kktix_dict["fail_list"], kktix_dict[
                        "played_sound_ticket"] = kktix_reg_new_main(
                            driver, config_dict, kktix_dict["fail_list"],
                            kktix_dict["played_sound_ticket"])
    ...
    if is_kktix_got_ticket:
        if '/events/' in config_dict[
                "homepage"] and '/registrations/' in config_dict[
                    "homepage"] and "-" in config_dict["homepage"]:
            # do nothing when second time come in.
            if len(url.split('/')) >= 7:
                if len(config_dict["homepage"].split('/')) >= 7:
                    # match event code.
                    if url.split('/')[4] == config_dict["homepage"].split(
                            '/')[4]:
                        # break loop.
                        is_kktix_got_ticket = False
    ...
    if is_kktix_got_ticket:
        if not kktix_dict["start_time"] is None:
            if not kktix_dict["done_time"] is None:
                bot_elapsed_time = kktix_dict["done_time"] - kktix_dict[
                    "start_time"]
...
def famiticket_main(driver, url, config_dict):
    ...
    if '/Home/User/SignIn' in url:
        ...
        fami_password = config_dict["advanced"][
            "fami_password_plaintext"].strip()
        if fami_password == "":
    ...
    if '/Home/Activity/Info/' in url:
        ...
        fami_dict["fail_list"] = fami_verify(driver, config_dict,
                                             fami_dict["fail_list"])
    else:
        fami_dict["fail_list"] = []
                ...
                if seq <= len(option_text_string):
                    ...
                    if direction == "right":
                        question_answer_char = option_text_string[
                            len(option_text_string) -
                            seq:len(option_text_string) - seq + 1]
                        ...
                        for each_option_div in each_option_items_div:
                            ...
                            if not option_content_div is None:
                                ...
                                if question_direction in ['left', 'right']:
                                    for answer_item in util.synonym_dict(
                                            question_answer_char):
                                        ...
                                        for i in range(int_answer_char - 1):
                                            for answer_item in util.synonym_dict(
                                                ...
                                                if '更多' in option_content_div_text and answer_item + '個' in option_content_div_text:
                                                    is_match_more_then = True
                                                if 'MORE THEN' in option_content_div_text.upper(
...
def urbtix_main(driver, url, config_dict):
    ...
    if '.hk/member-login' in url:
        ...
        urbtix_password = config_dict["advanced"][
            "urbtix_password_plaintext"].strip()
        if urbtix_password == "":
...
def check_modal_dialog_popup(driver):
    ...
    el_div = None
    try:
        el_div = driver.find_element(By.CSS_SELECTOR,
...
def cityline_cookie_accept(driver):
    is_btn_click = press_button(driver, By.CSS_SELECTOR,
                                '.cookieWrapper_closeBtn')
...
def cityline_close_second_tab(driver):
    try:
        ...
        if window_handles_count > 1:
            ...
            if len(page_title) > 0:
                driver.switch_to.window(driver.window_handles[0])
                if not (".cityline.com/" in page_title
                        and "https://" in page_title):
                    driver.close()
                    driver.switch_to.window(driver.window_handles[-1])
    except Exception as exc:
        pass
...
def cityline_main(driver, url, config_dict):
    ...
    if 'cityline.com/Login.html' in url:
        ...
        cityline_password = config_dict["advanced"][
            "cityline_password_plaintext"].strip()
...
def ibon_auto_ocr(driver, config_dict, ocr, away_from_keyboard_enable,
            ...
            if config_dict["area_auto_select"]["enable"]:
                ...
                if is_do_ibon_performance_with_ticket_number:
                    ...
                    if config_dict["ocr_captcha"]["enable"]:
                        ...
                        is_captcha_sent = ibon_captcha(driver, config_dict,
                                                       ocr, Captcha_Browser,
                                                       model_name)
...
def hkticketing_accept_cookie(driver):
    ...
    accept_all_cookies_btn = None
    try:
        accept_all_cookies_btn = driver.find_element(By.CSS_SELECTOR,
                                                     '#closepolicy_new')
    except Exception as exc:
        if show_debug_message:
    ...
    if not accept_all_cookies_btn is None:
        ...
        try:
            ...
            ) and accept_all_cookies_btn.is_displayed():
                is_visible = True
        except Exception as exc:
            pass
...
def hkticketing_date_password_input(driver, config_dict, fail_list):
    ...
    try:
        ...
        el_password_input = driver.find_element(By.CSS_SELECTOR,
                                                my_css_selector)
    except Exception as exc:
        pass
    ...
    if not el_password_input is None:
        ...
        if not inputed_value is None:
            ...
            if inputed_value == "":
                ...
                local_array = []
                user_guess_string = config_dict["advanced"][
                    "user_guess_string"]
...
def hkticketing_next_button_press(driver):
    ...
    try:
        el_btn = driver.find_element(
            By.CSS_SELECTOR,
            '#continueBar > div.chooseTicketsOfferDiv > button')
    except Exception as exc:
        print("find next button fail...")
        print(exc)
...
def hkticketing_hide_tickets_blocks(driver):
    ...
    try:
        ...
        actionBlock_divs = driver.find_elements(By.CSS_SELECTOR,
                                                selector_string)
    ...
    try:
        ...
        detailModuleCopy_divs = driver.find_elements(By.CSS_SELECTOR,
                                                     selector_string)
        if not detailModuleCopy_divs is None:
            driver.execute_script("arguments[0].innerHTML='';",
                                  detailModuleCopy_divs)
    except Exception as exc:
        pass
    ...
    try:
        ...
        mapWrapper_divs = driver.find_elements(By.CSS_SELECTOR,
                                               selector_string)
...
def hkticketing_url_redirect(driver, url, config_dict):
    is_redirected = False
    redirect_url_list = [
        'queue.hkticketing.com/hotshow.html', '.com/detection.aspx?rt=',
        '/busy_galaxy.'
    ]
    for idx in range(20):
...
def hkticketing_content_refresh(driver, url, config_dict):
    ...
    check_url_list = [
        ".com/default.aspx", ".com/shows/show.aspx?sh=", ".com/detection.aspx",
        "/entry-hotshow.", ".com/_Incapsula_Resource?"
    ]
    ...
    content_retry_string_list = [
        "Access Denied", "Service Unavailable", "The service is unavailable",
        "HTTP Error 500", "HTTP Error 503", "504 Gateway Time-out",
        "502 Bad Gateway", "An error occurred while processing your request",
        ...
        "Cannot generate SSPI context", "System.Data.SqlClient.Sql",
...
def softix_powerweb_main(driver, url, config_dict):
    ...
    if is_hkticketing_sign_in_page:
        hkticketing_account = config_dict["advanced"][
            "hkticketing_account"].strip()
        hkticketing_password = config_dict["advanced"][
            "hkticketing_password_plaintext"].strip()
            ...
            if is_event_page:
                if config_dict["date_auto_select"]["enable"]:
                    if not hkticketing_dict["is_date_submiting"]:
                        hkticketing_dict[
                            "is_date_submiting"], hkticketing_dict[
                                "fail_list"] = hkticketing_date_auto_select(
                                    driver, config_dict,
                                    hkticketing_dict["fail_list"])
                        pass
                    else:
...
def khan_go_buy_redirect(driver, domain_name):
    ...
    if 'kham.com' in domain_name:
        is_button_clicked = press_button(
            driver, By.CSS_SELECTOR,
            'div#content > p > a > button[onclick].red')
    if 'ticket.com' in domain_name:
        is_button_clicked = press_button(
            driver, By.CSS_SELECTOR,
            'div.row > div > a.btn.btn-order.btn-block')
...
def kham_product(driver, domain_name, config_dict):
    ...
    is_date_assign_by_bot = hkam_date_auto_select(driver, domain_name,
                                                  config_dict)
                                    ...
                                    if len(td_array) > 0:
                                        ...
                                        if ticket_count_text.isdigit():
                                            if int(
                                                ...
                                                if show_debug_message:
                                                    print(
                                                        "skip this row, because ticket_count available only:",
                                                        ticket_count_text)
                                                # skip this row.
                                                row_text = ""
...
def kham_switch_to_auto_seat(driver):
    ...
    try:
        ...
        btn_switch_to_auto_seat = driver.find_element(By.CSS_SELECTOR,
                                                      my_css_selector)
    except Exception as exc:
...
def kham_performance(driver, config_dict, ocr, Captcha_Browser, domain_name,
                     model_name):
    show_debug_message = True  # debug.
    show_debug_message = False  # online
    ...
    if auto_fill_ticket_number:
        ...
        # udn use reCaptcha.
        if not ('udnfunlife' in domain_name):
            is_captcha_sent = kham_captcha(driver, config_dict, ocr,
...
def kham_auto_ocr(driver, config_dict, ocr, away_from_keyboard_enable,
                  previous_answer, Captcha_Browser, ocr_captcha_image_source,
                  model_name):
    show_debug_message = True  # debug.
    show_debug_message = False  # online
...
def kham_main(driver, url, config_dict, ocr, Captcha_Browser):
    ...
    if 'udnfunlife' in domain_name:
        ...
        if 'https://tickets.udnfunlife.com/application/utk01/utk0101_.aspx' == url.lower(
            ...
            udn_password = config_dict["advanced"][
                "udn_password_plaintext"].strip()
            if udn_password == "":
        ...
        if 'utk0203_.aspx?product_id=' in url.lower():
            ...
            if not ticket_number_text is None:
                # layout format #1
                is_ticket_number_assigned = assign_text(
                    driver,
                    By.CSS_SELECTOR,
                    select_query,
                ...
                # layout format #2
                date_auto_select_enable = config_dict["date_auto_select"][
                    "enable"]
                if date_auto_select_enable:
                    kham_product(driver, domain_name, config_dict)
        ...
        if 'utk0222_02.aspx?product_id=' in url.lower():
            ...
            is_ticket_number_sent = assign_text(
                driver,
                By.CSS_SELECTOR,
                'input#QRY2',
                str(config_dict["ticket_number"]),
            ...
            if is_ticket_number_sent:
                is_fastbuy_pressed = press_button(driver, By.CSS_SELECTOR,
                                                  'input#f_btn')
        ...
        if '.aspx?performance_id=' in url.lower(
        ) and 'product_id=' in url.lower():
            model_name = url.split('/')[5]
            if len(model_name) > 7:
                model_name = model_name[:7]
            captcha_url = '/pic.aspx?TYPE=%s' % (model_name)
        ...
        if '.aspx?performance_id=' in url.lower(
        ) and 'performance_price_area_id=' in url.lower():
            model_name = url.split('/')[5]
            if len(model_name) > 7:
                model_name = model_name[:7]
            captcha_url = '/pic.aspx?TYPE=%s' % (model_name)
        ...
        if '/utk13/utk1306_.aspx' in url.lower():
            ...
            if config_dict["ocr_captcha"]["enable"]:
                ...
                kham_captcha(driver, config_dict, ocr, Captcha_Browser,
                             model_name)

                kham_account = config_dict["advanced"]["kham_account"]
                kham_password = config_dict["advanced"][
                ...
                ticket_password = config_dict["advanced"][
...
def ticketplus_assign_ticket_number(target_area, config_dict):
    ...
    try:
        ...
        ticket_number_div = target_area.find_element(By.CSS_SELECTOR,
                                                     my_css_selector)
    except Exception as exc:
        if show_debug_message:
                    ...
                    for i in range(add_count):
                        ...
                        try:
                            ...
                            if len(ticket_number_text) > 0:
                                ...
                                if ticket_number_text_int >= ticket_number:
                                    print(
                                        "match target ticket count (now/target):",
...
def ticketplus_order_expansion_auto_select(driver, config_dict,
                                           area_keyword_item,
            ...
            if ("Network.response" in network_log["method"]
                ...
                if 'request' in network_log["params"]:
                    if 'url' in network_log["params"]["request"]:
                        if url_keyword in network_log["params"]["request"][
                                "url"]:
                            url_list.append(
...
def ticketplus_order(driver, config_dict, ocr, Captcha_Browser,
    ...
    try:
        ...
        next_step_button = driver.find_element(By.CSS_SELECTOR,
                                               my_css_selector)
        if not next_step_button is None:
    ...
    if is_button_disabled:
        ...
        #print("is_price_assign_by_bot", is_price_assign_by_bot)
        if is_price_assign_by_bot:
            is_answer_sent, ticketplus_dict[
                "fail_list"], is_question_popup = ticketplus_order_exclusive_code(
                    driver, config_dict, ticketplus_dict["fail_list"])

            auto_submit = True
            if auto_submit:
                my_css_selector = "div.order-footer > div.container > div.row > div > button.nextBtn"
                is_form_sumbited = press_button(driver, By.CSS_SELECTOR,
                                                my_css_selector)
...
def ticketplus_auto_ocr(driver, config_dict, ocr, previous_answer,
                        Captcha_Browser):
    show_debug_message = True  # debug.
    show_debug_message = False  # online
    ...
    if len(config_dict["advanced"]["ticketplus_account"]) > 0:
        ...
        if not is_user_signin:
            ...
            try:
                ...
                sign_in_btn = driver.find_element(By.CSS_SELECTOR,
                                                  my_css_selector)
        ...
        if not el_pass is None:
            try:
                if el_pass.is_enabled():
                    inputed_text = el_pass.get_attribute('value')
                    if not inputed_text is None:
                        ticketplus_password = config_dict["advanced"][
...
def ticketplus_main(driver, url, config_dict, ocr, Captcha_Browser):
    ...
    if '/order/' in url.lower():
        ...
        if is_event_page:
            ...
            if not is_reloading:
                ticketplus_dict = ticketplus_order(driver, config_dict, ocr,
                                                   Captcha_Browser,
                                                   ticketplus_dict)
...
def facebook_main(driver, config_dict):
    ...
    facebook_password = config_dict["advanced"][
        "facebook_password_plaintext"].strip()
...
def get_current_url(driver):
    ...
    try:
        ...
        exit_bot_error_strings = [
            'Max retries exceeded', 'chrome not reachable',
            'unable to connect to renderer',
            'failed to check if window was closed',
            'Failed to establish a new connection', 'Connection refused',
            'disconnected', 'without establishing a connection',
...
def main(args):
    ...
    try:
        if config_dict["ocr_captcha"]["enable"]:
            ocr = ddddocr.DdddOcr(show_ad=False,
                                  beta=config_dict["ocr_captcha"]["beta"])
            Captcha_Browser = NonBrowser()
            if len(config_dict["advanced"]["tixcraft_sid"]) > 1:
                set_non_browser_cookies(driver, config_dict["homepage"],
                                        Captcha_Browser)
    except Exception as exc:
        print(exc)
        pass
    ...
    while True:
        ...
        if config_dict["advanced"]["reset_browser_interval"] > 0:
            ...
            if maxbot_running_time > config_dict["advanced"][
        ...
        if tixcraft_family:
            is_quit_bot = tixcraft_main(driver, url, config_dict, ocr,
                                        Captcha_Browser)
...
def cli():
    parser = argparse.ArgumentParser(description="MaxBot Aggument Parser")
    ...
    #default="False",
    parser.add_argument("--headless", help="headless mode", type=str)
    ...
    parser.add_argument("--window_size", help="Window size", type=str)
